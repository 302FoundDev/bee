...
export const DropDownProfile = () => {
  ...
  const menuItemStyle =
    "flex gap-1.5 items-center px-4 py-2 text-sm font-semibold rounded text-neutral-900 dark:text-neutral-100 hover:bg-neutral-100 dark:hover:bg-slate-800";
    ...
    <>
      {session ? (
        <div className="relative flex">
          ...
          {isDropOpen && (
            <div
              className="absolute right-0 z-20 px-2 py-2 mt-2 transition duration-300 ease-in-out origin-top transform scale-y-100 bg-white rounded-md shadow-lg opacity-100 border-neutral-800 top-5 w-60 dark:bg-neutral-950 dark:border-zinc-300"
            >
              <div className="px-4">
                <p className="text-[15px] text-neutral-900 font-semibold dark:text-neutral-100">
                  {user?.full_name}
                </p>
                <p className="mb-4 text-sm opacity-75 text-neutral-900 dark:text-neutral-100">
                  {user?.email}
                </p>
              </div>
